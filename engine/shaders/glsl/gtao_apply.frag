...
layout(binding = 9) uniform sampler2D waterTex;     // WaterTargetRT（R=mask, G=水面线性视距(m), BA=八面体世界法线；Nearest）

// Camera UBO（binding 10；本 pass 比常规后处理多一个 LUT 输入 + 水面 RT）
...
// ===== 硬编码水材质（常见清水 PBR 值，并入本 pass 合成以省一个全屏 pass）=====
...
void main() {
    ...
    // 折射 = 背景色 × Beer-Lambert 吸收 + 淡蓝 in-scatter；反射 = Fresnel(0.02) × 天空 + GGX 太阳高光。
    {
        ...
        if (w.r > 0.5 && centerDepth < 0.999999) {
            ...
            // 与中心像素同待遇：AO 乘进偏移采样（emissive 例外直通），避免折射底色比周围偏亮
            float refrEmis = texture(materialTex, refrUV).w;
            vec3 bottomLit = texture(lightTex, refrUV).rgb
                * (refrEmis > 0.01 ? 1.0 : texture(aoTex, refrUV).r);
            vec3 bottom = bottomLit * transmittance;
            ...
            // --- 水面 SSR：复用 metallic 分支的 DoSSR 射线步进 ---
            ...
            vec3 reflColor = skyRefl;
            {
                ...
                if (hitssr) {
                    // 反射源 = 已光照 composite（与 metallic SSR 同源），钳回屏内防 wrap 垃圾
                    vec3 ssrColor = texture(lightTex, clamp(hitUV, 0.001, 0.999)).rgb;
