...
// world.vert - 体素世界面渲染（从 OpenGL 版 voxel.vertex 迁移）
// 输入: quad 顶点 + 每面一个实例 (Chunk::FaceInstance: pos3 + packedData1)
...
layout(location = 0) in vec3 aPos;          // 单元 quad 顶点
layout(location = 1) in vec3 aFacePos;      // 面中心世界坐标
...
void main() {
    ...
    // 这里按旧版逻辑用 useZX 区分，让两个面旋转成不同的斜向十字（旧版 ALPHATEST 分支）
