// BreakoutGame.cpp - 打砖块色块原型(IGameModule 实现,独立插件 DLL,见 games/breakout)
// 挡板/球为场景树实体(assets/breakout.json),砖块网格启动时动态创建;
// 每帧同步实体位置,命中砖块销毁实体。
...
// ===== 插件导出(引擎 GameManager::LoadPlugin 约定;同 games/contact2d) =====
extern "C" __declspec(dllexport) const char* GetGameModuleName() {
    return "breakout"; // 与 assets/breakout.json 顶层 "game" 键一致
}
