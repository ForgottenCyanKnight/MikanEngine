...
// 资源场景树化: 蛇身/食物/背景/菜单 UI 均定义为场景树实体(见 scenes/snake.json),
...
// 注: SnakeGame 是独立插件 DLL(见本项目 games/snake),不使用 MIKAN_API(dllexport/dllimport)——
...
private:
    ...
    // ===== 场景树实体(scenes/snake.json 定义;动态逻辑每帧同步)=====
